...
const OverlayLayer = function OverlayLayer(options) {
  ...
  const layerIconCls = `round compact icon-small relative no-shrink light ${
    hasStylePicker ? "style-picker" : ""
  }`;
  const cls =
    `${clsSettings} flex row align-center padding-left padding-right-smaller item wrap`.trim();
  ...
  const toggleVisible = function toggleVisible(visible) {
    ...
    const groupExclusive =
      viewer.getGroup(layerGroup) && viewer.getGroup(layerGroup).exclusive;
  ...
  const extendedLegendContent = Component({
    render() {
      ...
      return `<div id="${this.getId()}" class="padding-left">${
        hasExtendedLegend ? Legend(style) : ""
      }</div>`;
    },
    setContent(content) {
  ...
  const extendedLegendCmp = Collapse({
    collapseX: false,
    contentComponent: extendedLegendContent,
  ...
  const label = Component({
    ...
    render() {
      const labelCls =
        "text-smaller padding-x-small grow pointer no-select overflow-hidden basis-50";
  ...
  const layerInfoMenuItem = Component({
    onRender() {
      ...
      labelEl.addEventListener("click", (e) => {
        ...
        document
          .getElementById(moreInfoButton.getId())
          .dispatchEvent(eventOverlayProps);
        e.preventDefault();
      });
    },
    render() {
      const labelCls =
        "text-smaller padding-x-small grow pointer no-select overflow-hidden";
  ...
  if (layer.get("zoomToExtent")) {
    const zoomToExtentMenuItem = Component({
      onRender() {
        ...
        labelEl.addEventListener("click", (e) => {
          const extent =
            typeof layer.getSource !== "undefined" &&
            typeof layer.getSource().getExtent !== "undefined"
  ...
  if (layer.get("exportable")) {
    ...
    const formats = exportFormatArray
      .map((format) => format.toLowerCase())
      .filter(
        (format) => format === "geojson" || format === "gpx" || format === "kml"
      );
    if (formats.length === 0) {
  ...
  return Component({
    ...
    onAdd(evt) {
      ...
      if (
        ...
        map.on("moveend", () => {
          if (
            map.getView().getResolution() > layer.getMinResolution() &&
            map.getView().getResolution() < layer.getMaxResolution()
          ) {
